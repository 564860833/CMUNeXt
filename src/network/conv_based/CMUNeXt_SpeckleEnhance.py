...
class DDSR(nn.Module):
    ...
    def __init__(self, channels, smooth_k=5, alpha_init_raw=-5.3, max_scale=0.05):
        super().__init__()
        if max_scale < 0:
...
class CMUNeXt_SpeckleEnhance(nn.Module):
    def __init__(
        ...
        invalid_stages = self.ddsr_stages - {0, 1, 2, 3}
        if invalid_stages:
            raise ValueError(f"Unsupported DDSR stages: {sorted(invalid_stages)}")
...
def cmunext_speckle(
    ...
    """Default: shallow DDSR on stages 0 and 1."""
...
def cmunext_speckle_shallow(input_channel=3, num_classes=1, ddsr_max_scale=0.05, ddsr_skip_only=True):
...
def cmunext_speckle_deep(input_channel=3, num_classes=1, ddsr_max_scale=0.05, ddsr_skip_only=True):
...
def cmunext_speckle_s(input_channel=3, num_classes=1, ddsr_max_scale=0.05, ddsr_skip_only=True):
    return CMUNeXt_SpeckleEnhance(
        input_channel=input_channel, num_classes=num_classes,
        dims=(8, 16, 32, 64, 128),
        depths=(1, 1, 1, 1, 1),
...
def cmunext_speckle_l(input_channel=3, num_classes=1, ddsr_max_scale=0.05, ddsr_skip_only=True):
    return CMUNeXt_SpeckleEnhance(
        input_channel=input_channel, num_classes=num_classes,
...
if __name__ == "__main__":
    ...
    for name, mod in model.ddsr_modules.items():
        scale = mod.max_scale * torch.sigmoid(mod.alpha)
